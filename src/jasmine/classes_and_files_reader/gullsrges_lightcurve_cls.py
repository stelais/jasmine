...
import src.jasmine.classes_and_files_reader.gullsrges_reader as grr
import pandas as pd
...
class LightcurveEventGULLSRGES:
    def __init__(self, master_file_index, data_folder='../data'):
        ...
        # No data challenge LC number in this sample, so we should just identify each event by it's order in the sample.csv file.
    ...
    def lightcurve_data(self):
        ...
        :return:
        """
        lightcurve_df_list = grr.lightcurve_data_reader(self.event_info.event_subrun, self.event_info.event_id,
                                                        folder_path_=self.data_folder)
    ...
    def microlensing_signal(self):
        ...
        :return:
        """
        microlensing_df_list = grr.microlensing_signal_reader(self.event_info.event_subrun, self.event_info.event_id,
                                                              folder_path_=self.data_folder)
        return microlensing_df_list


class LightcurveEventGULLSRGES_NameBased:
    def __init__(self, lightcurve_name, sample_folder='../data', master_file_path='../data/OMPLLD_croin_cassan_0.sample.csv'):
        # SIS: The light curve name is based on Subrun and EventID. I would use its name as a reference
        # instead of creating other indexes. This because it will be easier to validate and not rerun
        # same events when using the actual full data.
        master_df = pd.read_csv(master_file_path)
        self.lightcurve_name = lightcurve_name
