...
def creating_all_directories(*, event_type_
    ...
    while i <len(event_type_):
        if event_type_[i].lower() not in valid_names:
            name = input(f' Event type {event_type_[i]} not recognized.\nPlease re-enter one of these names {valid_names} or type exit to close the program.\n')
            if name.lower() == 'exit': return None
            else: 
            ...
            if event_type_[i] == 'all' and len(event_type_)>1:
                event_type_ = ['all']; break 
        i+=1
    if 'all' in event_type_: event_type_ = valid_names[0:4] # 
